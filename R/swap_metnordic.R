...
#' function.
#'
#' @param dldir download directory containing data and metadata, as generated by `metnordic_csv()`
...
swap_metnordic <- function(dldir, outpath, timescale = "daily", verbose = FALSE){
  ...
  SWAP_INPUT_DF <- SWAP_INPUT_DF %>% group_by(.data$DATE) %>% mutate(
    RAD = sum(.data$hRAD, na.rm = T) %>% round(2),
    Tmin = min(.data$hTEMP, na.rm = T) %>% round(2),
    Tmax = max(.data$hTEMP, na.rm = T) %>% round(2),
    HUM = mean(.data$hHUM, na.rm = T) %>% round(2),
    WIND = mean(.data$hWIND, na.rm = T) %>% round(2),
    RAIN = sum(.data$hRAIN, na.rm = T) %>% round(2)
  ) %>%
