...
# read in file paths of the chosen location
  ...
  package_env <- new_environment()


  models <- list.files(paste0(modelled_climate_data_path,"/KSSData"))
  files <-
    list.files(
      paste0(modelled_climate_data_path, "/KSSData"),
      recursive = T,
      full.names = T
    )


  cat("\r transforming NetCDF to dataframe...")

  nc_file <- ncdf4::nc_open(files[6464])

  vars<- nc_file$var %>% names()

  getvar <- ncdf4::ncvar_get(nc_file, vars[1])

  transposed <- apply(getvar, FUN = mean, MARGIN = 3) %>% round(2)

  vroom::vroom(files, id = "path", col_names = F)

  package_env$climate_data <-
