...
cwatm_hourly_to_daily_ncdf4 <- function(inpath, outpath, verbose = FALSE){
  ...
  for (today in date_only) {
    ...
    mt_print(verbose, "cwatm_hourly_to_daily_ncdf4","converting day...",paste0(today, " (", round(cati/length(date_only),4)*100, "%)"), rflag = TRUE)
...
convert_to_cwatm <- function(input, output, verbose = FALSE){
  ...
  # UNSTABLE!
  mt_print(verbose, "convert_to_cwatm", "parsing time dimension defitions...")
  ...
  mt_print(verbose, "convert_to_cwatm", "loading files...", length(list.files(input)))
