...
while((dl_stop == FALSE) & (dl_attempt < 50)){
  ...
  if(dl_stop == FALSE){
    for (url in to_download) {
      ...
      print(status)
      # if the download fails, just skip it in the loop, and get it the next
      # time around
      tryCatch({
        metnordic_download(url = url ,outdir = mn_download_dir,
                           vars = cwatm_raw_vars, overwrite = T,
                           preview = F)
        i = i+1
      },
      error = function(cond) {
        message(paste("failed!", i))
        NA
      })
    }
  }
