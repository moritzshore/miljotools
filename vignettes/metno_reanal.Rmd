...
# Aggregating in parallel fashion:
require(doParallel)
cores <- detectCores() - 2
...
If you would like to re-project your now merged NetCDF files, you can
use this function:

```{r, eval = runmetnovig}
to_fp <- str_replace(merged_fp, "merged", "projected")
dir.create(paste0(mydir, "/projected"), showWarnings = F)

proj_fp <- metnordic_reproject(filepath = merged_fp,
                               outfile = to_fp,
...
require(terra)
require(sf)
map <- terra::rast(proj_fp)
provided_extent <- sf::read_sf(cs10_basin)
mapview::mapview(map, legend = FALSE)+mapview::mapview(provided_extent, alpha.regions = .5, legend = FALSE)
```
...
```{r gmnrv3ncdf4,  eval = runmetnovig}
...
As one can read, it is a long process to get to this point. Therefore,
all these steps will be included in a final function which links them
all together (at some point, similar to `swat_weather_input_chain()`)
...
list.files(pattern = "met_analysis*") %>% file.remove()
